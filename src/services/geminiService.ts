...
<<<<<<< HEAD
 * CACHE MANAGER
 * Handles creation and retrieval of Gemini Context Caches via REST API
 * (Since Node.js SDK FileManager doesn't work in browser)
 */
async function getOneTimeCache(modelName: string): Promise<string> {
  const API_KEY = process.env.VITE_GEMINI_API_KEY || process.env.API_KEY || '';
  if (!API_KEY) throw new Error("No API Key found");

  const CACHE_DISPLAY_NAME = "bingo-product-db-v1";

  // 1. Check if exists
  try {
    const listResp = await fetch(`https://generativelanguage.googleapis.com/v1beta/cachedContents?key=${API_KEY}`);
    if (listResp.ok) {
      const listData = await listResp.json();
      if (listData.cachedContents) {
        const existing = listData.cachedContents.find((c: any) => c.displayName === CACHE_DISPLAY_NAME);
        if (existing) {
    ...
    console.warn("Failed to list caches", e);
  }

  // 2. Create New Cache
  console.log("Creating new Context Cache...");
  // Get FULL database
  const fullDb = productService.getFullDatabaseString();

  const createResp = await fetch(`https://generativelanguage.googleapis.com/v1beta/cachedContents?key=${API_KEY}`, {
    method: 'POST',
    ...
    body: JSON.stringify({
      ...
      contents: [{
        role: "user",
        parts: [{ text: JSON.stringify(JSON.parse(fullDb)) }] // Ensure it's valid stringified JSON text
      }]
    })
  });
  ...
  if (!createResp.ok) {
    ...
    throw new Error(`Cache creation failed: ${createResp.status} ${err}`);
  }
...
export const generateBoq = async (answers: Record<string, any>): Promise<Boq> => {
  ...
  try {
    ...
    cachedContentName = await getOneTimeCache(modelName);
    console.log("Using Cache:", cachedContentName);
  } catch (e) {
    ...
    **STEP 2 - IF FOUND IN DATABASE (PRICE HANDLING):**
    ...
    - **Price Check:**
        - If 'price' is 0, null, or missing: **YOU MUST STILL USE THE PRODUCT.**
            - **MANDATORY:** Do NOT fall back to web just because price is missing.
            - Estimate the price using your web knowledge (INR).
...
`;
  ...
  try {
    ...
    // If cache is active, we DON'T need to send the database string again.
    // If cache failed, we use the fallback filtered list.
    if (!cachedContentName) {
      const curatedDatabaseString = productService.getFilteredDatabaseString(allowedCategories);
      promptParts.push({ text: `Custom Product Database (Filtered for Relevance): ${curatedDatabaseString}` });
      console.log("Using Fallback Filtered Database (No Cache)");
    } else {
      // With cache, we just remind the model it has the data
      promptParts.push({ text: `[SYSTEM] You have access to the FULL Product Database via Context Caching. Search it thoroughly.` });
    }
    ...
    if (cachedContentName) {
      requestOptions.cachedContent = cachedContentName;
    }
