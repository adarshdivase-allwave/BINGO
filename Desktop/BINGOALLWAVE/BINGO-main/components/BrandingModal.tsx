...
const BrandingModal: React.FC<BrandingModalProps> = ({ isOpen, onClose, settings, onSave }) => {
  ...
  const handleSecondaryLogoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      const reader = new FileReader();
      reader.onloadend = () => {
        setLocalSettings(prev => ({ ...prev, secondaryLogoUrl: reader.result as string }));
      };
      reader.readAsDataURL(file);
    }
  };
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-2xl border border-slate-200 dark:border-slate-700 flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-8 overflow-y-auto space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label className="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-2">Left Header Logo (Primary)</label>
                    <div className="mt-1 flex items-center">
                        <span className="inline-block h-20 w-32 rounded-md overflow-hidden bg-slate-100 dark:bg-slate-700 border border-slate-300 dark:border-slate-600 flex items-center justify-center">
                            {localSettings.logoUrl ? (
                                <img src={localSettings.logoUrl} alt="Primary Logo" className="max-h-full max-w-full object-contain" />
                            ) : (
                                <span className="text-xs text-slate-400">No Logo</span>
                            )}
                        </span>
                        <label htmlFor="logo-upload" className="ml-3 bg-white dark:bg-slate-700 py-2 px-3 border border-slate-300 dark:border-slate-600 rounded-md shadow-sm text-sm leading-4 font-medium text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 cursor-pointer inline-flex items-center">
                            <UploadIcon />
                    ...
                    <p className="text-xs text-slate-500 mt-1">Upload your main company logo (e.g., All-Wave + 25 Years badge). This will appear on the <strong>Left</strong>.</p>
                </div>

                <div>
                    <label className="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-2">Right Header Logo (Secondary)</label>
                    <div className="mt-1 flex items-center">
                        <span className="inline-block h-20 w-32 rounded-md overflow-hidden bg-slate-100 dark:bg-slate-700 border border-slate-300 dark:border-slate-600 flex items-center justify-center">
                            {localSettings.secondaryLogoUrl ? (
                                <img src={localSettings.secondaryLogoUrl} alt="Secondary Logo" className="max-h-full max-w-full object-contain" />
                            ) : (
                                <span className="text-xs text-slate-400">No Logo</span>
                            )}
                        </span>
                        <label htmlFor="sec-logo-upload" className="ml-3 bg-white dark:bg-slate-700 py-2 px-3 border border-slate-300 dark:border-slate-600 rounded-md shadow-sm text-sm leading-4 font-medium text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 cursor-pointer inline-flex items-center">
                            <UploadIcon />
                            <span className="ml-2">Upload</span>
                            <input id="sec-logo-upload" name="sec-logo-upload" type="file" className="sr-only" onChange={handleSecondaryLogoUpload} accept="image/png, image/jpeg, image/svg+xml" />
                        </label>
                    </div>
                    <p className="text-xs text-slate-500 mt-1">Upload partner logos (e.g., PSNI, AVIXA). This will appear on the <strong>Right</strong>.</p>
                </div>
            </div>
            ...
            <div>
                 ...
                 <div className="flex items-center gap-2">
                    <input type="color" id="primaryColor" value={localSettings.primaryColor} onChange={handleColorChange} className="h-9 w-9 p-1 border border-slate-300 rounded cursor-pointer" />
                    <input type="text" value={localSettings.primaryColor} onChange={handleColorChange} className="block w-24 bg-slate-100 dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-md shadow-sm py-2 px-3 text-slate-900 dark:text-white sm:text-sm font-mono" maxLength={7} />
                 </div>
            </div>
